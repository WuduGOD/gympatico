...
const { getUserPlan } = require('../utils/userHelpers'); // <--- IMPORT HELPERA (DRY)
...
const checkLimits = (resourceType) => {
  return async (req, res, next) => {
    ...
    try {
      // Pobieramy plan za pomocą ujednoliconego helpera
      ...
      // WERYFIKACJA LIMITÓW DLA UŻYTKOWNIKÓW Z POZIOMU FREE
      ...
      if (resourceType === 'workouts') {
        ...
        if (parseInt(countRes.rows[0].count, 10) >= 10) {
          return res.status(403).json({ 
            error: "Osiągnięto limit planu darmowego (maksymalnie 10 treningów). Odblokuj wersję Premium 🦾 lub usuń stare sesje, aby zapisać nową!" 
          });
        }
      }
      ...
      // D. Limit Znajomych (Maksymalnie 5 zaakceptowanych znajomości w gangu)
